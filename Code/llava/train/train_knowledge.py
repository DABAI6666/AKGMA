...
def preprocess_multimodal(
    ...
    triple_prompt = ""
    if triples:
        triple_prompt = "Given the associating structured triple from a knowledge graph of this image: " + ", ".join(triples) + ". "

    for source in sources:
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value']:
                sentence['value'] = sentence['value'].replace(DEFAULT_IMAGE_TOKEN, '').strip()
                sentence['value'] = triple_prompt + DEFAULT_IMAGE_TOKEN + '\n' + sentence['value']
...
def preprocess_llama_2(
    ...
    triple_prompt = ""
    if triples:
        triple_prompt = "Given the associating structured triple from a knowledge graph of this image: " + ", ".join(
            triples) + ". "
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            sentence["value"] = triple_prompt + sentence["value"]
...
def preprocess_plain(
    ...
    """
    Plain preprocessing without complex formatting, with optional knowledge triples.
    Concatenates conversations and adds knowledge triples if provided.
    """
    # 拼接三元组信息
    triple_prompt = ""
    if triples:
    ...
    for source in sources:
        ...
        # 在对话内容前拼接三元组信息
        conversation = triple_prompt + source[0]['value'] + source[1]['value'] + conversation_lib.default_conversation.sep
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.PLAIN:
        return preprocess_plain(sources, tokenizer ,triple_prompt)
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_2:
        return preprocess_llama_2(sources, tokenizer, triple_prompt,has_image=has_image)
    if conversation_lib.default_conversation.version.startswith("v1"):
        return preprocess_v1(sources, tokenizer, triple_prompt, has_image=has_image)
    ...
    for source in sources:
        ...
        full_conversation = triple_prompt + conversation
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        triples = self.triple_data.get(image_id, [])
        selected_triples = triples if len(triples) <= 10 else random.sample(triples, 10)  # 随机选择最多10个三元组
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, opt_input_ids = tuple([instance[key] for instance in instances]
                                  for key in ("input_ids", "labels", "opt_input_ids"))
